  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Delete materie ( {{ $pm->id }} )</h4>
      </div>
      <div class="modal-body">
        {{ Form::open(['url'=> URL::route('profesor_materii_delete'), 'class' => 'form']) }}
        {{ Form::hidden('pm_id', $pm->id)}}
        {{ Form::hidden('id', $profesor->id)}}
        Esti sigur ca vrei sa stergi materia "{{ $pm->denumirea }}" a profesorului "{{ $profesor->nume . ' ' . $profesor->prenume }}"?
      </div>
